...
```text
[Insertar aqui la tabla resumen construida a partir de `trabajo_carlos/1. optimizacion_numerica/heuristicos/datos/resumen_punto_2_heuristicos.csv`]
```
...
```text
[Insertar imagen: grafica de barras del mejor valor promedio por metodo en 3D]
```

#### Evaluaciones promedio por metodo

Esta figura debe comparar el costo computacional promedio de los tres metodos, medido en numero de evaluaciones de la funcion objetivo. Su lectura complementa la figura anterior, ya que permite discutir si los mejores resultados se obtienen a costa de un esfuerzo computacional mayor o si existe tambien una ventaja en eficiencia.

```text
[Insertar imagen: grafica de barras de evaluaciones promedio por metodo]
```

#### Vista resumida tipo mapa de calor o tabla coloreada

Como apoyo adicional, puede incluirse una visualizacion compacta en la que las filas representen cada combinacion `funcion-dimension` y las columnas correspondan a los tres metodos. El valor mostrado puede ser el mejor valor promedio o un ranking relativo entre metodos. Esta figura resulta util para identificar de un vistazo en que funciones cada algoritmo se comporta mejor.

```text
[Insertar imagen: mapa de calor o tabla coloreada comparando metodos por funcion y dimension]
```

### Interpretacion general de los resultados heuristicos

En terminos generales, los resultados obtenidos muestran que `PSO` y `evolucion diferencial` ofrecen un mejor desempeno global que el algoritmo evolutivo simple en la mayoria de funciones estudiadas. En particular, `evolucion diferencial` presenta un comportamiento muy fuerte en funciones como `Rosenbrock` y `Rastrigin`, mientras que `PSO` logra soluciones muy competitivas y consistentes en varios escenarios.

El algoritmo evolutivo tambien encuentra soluciones razonables, pero en promedio queda por debajo de los otros dos metodos en calidad de la solucion final. Aun asi, su presencia en el estudio es importante porque representa una estrategia poblacional clasica y permite comparar tres filosofias diferentes de exploracion del espacio de busqueda.

Finalmente, la interpretacion de esta subseccion debe conectarse con la discusion general del trabajo. No basta con identificar que metodo produce el menor valor promedio; tambien es necesario contrastar ese resultado con el numero de evaluaciones realizadas y con la dificultad propia de cada funcion. Esta comparacion sera clave mas adelante para responder que aportan los metodos heuristicos frente al descenso por gradiente.
